...
export function encodeAction(action: "page" | "season" | "variant", state: PanelState): string {
 ...
 */
export async function listSeasons(variant: RankingVariant): Promise<SeasonRef[]> {
  if (variant === "rpb") {
    // BTS seasons, aligned with /rankings UI.
    // S1 = BTS 1, S2 = BTS 2 → 3 (BTS 4 non encore débuté).
    return [
      { key: "2", label: "Saison 2", sublabel: "BTS 2 → 3" },
  ...
  if (variant === "wb") {
    ...
    return rows.map((r) => ({
      ...
      sublabel: r.season === 1 ? "UB 1 → 11" : r.season === 2 ? "UB 12 → 22" : undefined,
    }));
  }
  ...
  return rows.map((r) => ({
    key: String(r.season),
    label: `Saison ${r.season}`,
    sublabel: r.season === 1 ? "BBT 1 → 11" : r.season === 2 ? "BBT 12 → 22" : undefined,
  }));
}

/** Return the default season key for a variant (used on first render). */
export async function defaultSeasonKey(variant: RankingVariant): Promise<string> {
...
export async function renderRankingPanel(state: PanelState): Promise<PanelRenderResult> {
  const { variant } = state;
  ...
  const currentSeason = seasons.find((s) => s.key === state.season) ?? seasons[0];
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(`Page ${page + 1} / ${totalPages} · ${total} blader${total > 1 ? "s" : ""}`)
  ...
  components.push(
    new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder()
        .setCustomId(encodeAction("page", { ...state, page: Math.max(0, page - 1) }))
        .setLabel("◀ Précédent")
      ...
      new ButtonBuilder().setLabel("Voir sur le site").setURL(meta.url).setStyle(ButtonStyle.Link),
    ),
  );
